...
- **直读**：定位问题、收集证据时直接读取 prod 下的 log/data，**不 cp、不建副本**；引用证据时带文件路径 + 行号
...
PM 可能运行在 tmux 会话中、由 nanobot 程序驱动、用户实际在飞书远程交互（用户看到的每句话都是 `capture-pane` 文本快照转发，答复经 `send-keys` 文本回传）。以下规则无条件遵守——它们在用户直接坐在终端前的场景同样成立，代价极低：

1. **决策用文本编号选项**：需要用户决策时，输出编号选项列表（`方案 1：...` / `方案 2：...`，标注推荐项），请求回复编号。禁用 AskUserQuestion 工具——交互选择组件无法被 nanobot 程序化可靠操作（capture-pane 抓取的是渲染残影，send-keys 自由文本命中不了组件的选中逻辑）
2. **每轮输出自包含**：引用此前的方案、选项、数据时必须完整重述关键内容，禁止"如上面的方案 2"这类依赖终端历史的表述——飞书消息独立成条，用户看不到回滚
3. **简洁**：输出会被转发为飞书消息，控制单轮长度：结论先行，细节分层
4. **状态查询直答**：用户询问进度时，基于 `.features/index.yaml`、`.issues/index.yaml` 状态与最近 `git log` 简洁作答，不展开长篇
